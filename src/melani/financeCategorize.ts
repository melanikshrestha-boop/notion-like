...
const RULES: { category: string; match: RegExp }[] = [
  { category: "Income", match: /\b(payroll|direct dep|salary|venmo cashout|zelle from|irs treas|refund|interest paid)\b/i },
  ...
  { category: "Food / groceries", match: /\b(whole foods|trader joe|costco|walmart|target|aldi|kroger|safeway|grocery|instacart|fresh direct|wegmans)\b/i },
  { category: "Restaurants / coffee", match: /\b(starbucks|dunkin|mcdonald|chipotle|doordash|uber eats|grubhub|seamless|restaurant|cafe|coffee|pizza|sushi|bagel)\b/i },
  { category: "Transport", match: /\b(uber|lyft|mta|metrocard|omny|shell|exxon|chevron|gas station|parking|toll|ezpass|citi bike)\b/i },
  { category: "Health", match: /\b(pharmacy|cvs|walgreens|rite aid|doctor|dental|hospital|labcorp|quest diag|health|medical|insurance)\b/i },
  ...
  { category: "Transfers", match: /\b(transfer|payment thank|credit card payment|ach|wire|venmo|zelle|cash app|paypal)\b/i },
